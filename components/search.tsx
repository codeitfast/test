...
export default function Search(props:any){
  ...
  const loadingBox = <div className="font-black text-6xl absolute mx-auto opacity-50 rounded-md w-20 h-20 bg-transparent justify-self-center left-0 right-0 my-2"><AiOutlineLoading3Quarters className="animate-spin absolute left-0 right-0 ml-2 mt-2 text-black" /></div>
    return(
      ...
      <div className="w-full relative">
        <input value={props.inputValue} placeholder="Keywords, Themes, Etc..." onChange={(event)=>{
          ...
          }} className={`transition-all w-full p-4 border-gray-400 rounded-lg px-4 outline outline-1 outline-black ${err && 'outline outline-2 outline-red-600'}`} style={{backgroundColor: 'white' }}/>
    ...
    <button className={`transition-all rounded-md absolute top-0 bottom-0 my-auto right-2 h-10 w-10`} onClick={()=>{
      ...
      props.clear('')
      props.data[1]('')
    }} style={{background: props.colors.front}}><BiPaperPlane className={`mx-auto text-3xl transition-all ${clicked && 'animate-wiggle'}`} style={{color: props.colors.text}} onAnimationEnd={()=>setClicked(false)}/></button>
    </div>
    ...
    {props.loadingData && loadingBox}
          ...
          {(props.data[0].length > 0) && <div className="bg-white text-black p-2 rounded-md shadow-2xl  my-2 break-words styled-bubble box">this is where the ai chat would be. this is where the ai chat would be. this is where the ai chat would be. this is where the ai chat would be. this is where the ai chat would be. this is where the ai chat would be. this is where the ai chat would be.</div>}
        {(props.data[0].length > 0) && props.data[0].map((book:any, num:any) => {
